import pygame
import random
from pygame.locals import *
import sys

pygame.init()
fpsClock = pygame.time.Clock()

width = 1280
height = 600

surface = pygame.display.set_mode((width, height))

altura = 150

rojo_img = pygame.image.load("fondo_izq.png")
azul_img = pygame.image.load("fondo_der.png")

fondo_w = 720

linea_img = pygame.image.load("linea.png")

piedra_img = pygame.image.load("boton_piedra.png")
papel_img = pygame.image.load("boton_papel.png")
tijera_img = pygame.image.load("boton_tijera.png")

img_width = 162
img_hw = img_width/2

rafa_w = 172
rafa_h = 160

contador_img = pygame.image.load("contador.png")

contador_w = 146

pygame.font.init()

fuente = pygame.font.Font("Roboto.ttf", 48)

fg_img = pygame.image.load("fg.png")

tirada = 0
jugado = False
estado = -1
jugada = ''
ronda = 100

score_jugador = 0
score_ia = 0

while True:       
    surface.blit(rojo_img, (0,0))
    surface.blit(azul_img, (width/2,0))
    
    surface.blit(linea_img, (width/2-2,0))
            
    piedra_rect = surface.blit(piedra_img, (width/4-img_width-img_hw, height/2-img_hw))
    papel_rect = surface.blit(papel_img, (width/4-img_hw, height/2-img_hw))
    tijera_rect = surface.blit(tijera_img, (width/4+img_hw, height/2-img_hw))
    
    surface.blit(contador_img, (width/2-200, 20))

    contador = fuente.render(f'YO: {str(score_jugador)} - PC: {str(score_ia)}', True, (0,0,0))
    surface.blit(contador, (width/2-150, 100))
    
    
    if jugado:
        if estado == -1:
            message = fuente.render("", True, (255,255,255))
        elif estado == 0:
            message = fuente.render("Empate", True, (255,255,255))
        elif estado == 1:
            message = fuente.render("Ganaste", True, (255,255,255))
        elif estado == 2:
            message = fuente.render("Perdiste", True, (255,255,255))
        elif estado == 3:
            message = fuente.render("GAME OVER", True, (255,255,255))
            
        surface.blit(fg_img, (0,0))
        surface.blit(message, (width/2, height/2))
        
        
    if estado == 3:
        message = fuente.render("JUEGO FINALIZADO", True, (242,103,62))
        
        if score_jugador == score_ia:
            winner = fuente.render("-*- EMPATE -*-", True, (255,255,255))
        elif score_jugador > score_ia:
           winner = fuente.render("-*- VICTORIA -*-", True, (0,255,0))
        else:
            winner = fuente.render("-*- PERDISTE -*-", True, (203,40,33))

        surface.blit(fg_img, (0,0))
        surface.blit(message, (width/2, height/2))
        surface.blit(winner, (width/2, height/2+92))
        

    if tirada == 1:
        ia_rect = surface.blit(piedra_img, (x,y))
    elif tirada == 2:
        ia_rect = surface.blit(papel_img, (x,y))
    elif tirada == 3:
        ia_rect = surface.blit(tijera_img, (x,y))
        
    if jugada == 'piedra':
        piedra_rect = surface.blit(piedra_img, (width/4-img_width-img_hw, height/2-img_hw))
    elif jugada == 'papel':
        papel_rect = surface.blit(papel_img, (width/4-img_hw, height/2-img_hw))
    elif jugada == 'tijera':
        tijera_rect = surface.blit(tijera_img, (width/4+img_hw, height/2-img_hw))
    
    
    
    for event in pygame.event.get():
        if event.type == QUIT:
            pygame.quit()
            sys.exit()
        elif event.type == pygame.MOUSEBUTTONDOWN:
            pos = pygame.mouse.get_pos()
            x_pos,y_pos = event.pos
            jugado = False
            jugada = ''
            tirada = 0
            
            if estado == 3:
                ronda = 4
                score_jugador = 0
                score_ia = 0
            
            if estado != -1:
                estado = -1
                ronda = ronda - 1
                
                if ronda == 0:
                    estado = 3
            elif piedra_rect.collidepoint(x_pos,y_pos):
                jugada = "piedra"
                jugado = True
            elif papel_rect.collidepoint(x_pos,y_pos):
                jugada = "papel"
                jugado = True
            elif tijera_rect.collidepoint(x_pos,y_pos):
                jugada = "tijera"
                jugado = True
            else:
                None

            if jugado:
                tirada = random.randint(1,3)
                x,y = width*3/4-img_hw, height/2-img_hw

                if tirada == 1: #piedra
                    if jugada == "piedra":
                        #Empate
                        estado = 0
                    elif jugada == "papel":
                        #Ganaste
                        score_jugador = score_jugador + 1
                        estado = 1
                    else:
                        #Perdiste
                        score_ia = score_ia + 1
                        estado = 2
                elif tirada == 2: #papel
                    if jugada == "papel":
                        #Empate
                        estado = 0
                    elif jugada == "tijera":
                        #Ganaste
                        score_jugador = score_jugador + 1
                        estado = 1
                    else:
                        #Has perdido
                        score_ia = score_ia + 1
                        estado = 2
                elif tirada == 3: #tijera
                    if jugada == "tijera":
                        #Empate
                        estado = 0
                    elif jugada == "piedra":
                        #Ganaste
                        score_jugador = score_jugador + 1
                        estado = 1
                    else:
                        #Perdiste
                        score_ia = score_ia + 1
                        estado = 2
    altura += 1
    
    pygame.display.update()
    fpsClock.tick(30)